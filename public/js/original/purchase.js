$(document).ready(function() {
    ...
    // Fix scroll & z-index whenever contact_modal is stacked on top of another modal
    $(document).on('shown.bs.modal', '.contact_modal', function() {
        $(this).css({ 'overflow-y': 'auto', 'z-index': 1650 });
        $('.modal-backdrop').last().css('z-index', 1640);
    });
...
$(document).ready(function () {
    ...
    var __ocr_trans = {
    no_file:       'Importer une facture.',
    no_items:      'Aucun produit trouvé' ,
    error:         'messages.something_went_wrong',
    items_added:   "Produits ajoutés",
    ...
    // Fix scroll when stacked on top of invoice_scan_modal
    $(this).css({ 'overflow-y': 'auto', 'z-index': 1650 });
    $('.modal-backdrop').last().css('z-index', 1640);
    ...
    productSelect += '<button type="button" class="btn btn-primary btn-flat ocr-quick-add" data-name="' + escapeHtml(item.ocr_text || '') + '" title="Ajouter le produit"><i class="fa fa-plus"></i></button>';
    productSelect += '</span>';
}
    productSelect += '</div>';
